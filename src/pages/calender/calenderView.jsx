...
//   );
//   // Reverted to original parameters which seem correct for the API call to succeed
...
//   useEffect(() => {
//     // ✅ ROBUST CHECK: Using optional chaining and nullish coalescing for safety
//     const rawDailyWork = getDailyWorkData?.data?.data || [];
    
//     if (Array.isArray(rawDailyWork)) {
//       setDailyWork(rawDailyWork);
//     } else if (rawDailyWork?.data && Array.isArray(rawDailyWork.data)) {
//       // Fallback check if the data is wrapped an extra layer (e.g., { data: [tasks] })
//       setDailyWork(rawDailyWork.data);
//     } else {
//       setDailyWork([]);
//     }
    
//     // Force update only when data is successfully fetched to avoid multiple re-renders
//     if (getDailyWorkData && calendarRef.current) {
//         calendarRef.current.forceUpdate();
//     }
    
//   }, [getDailyWorkData]); // Dependency is solely on the fetch result

// useEffect(() => {
//   // ✅ ROBUST DATA ACCESS: Use safe access with empty array fallback
...
//   const currentDailyWork = Array.isArray(dailyWork) ? dailyWork : [];
...
//       console.warn(`Invalid color detected: ${color}, using default: ${defaultColor}`);
//       return defaultColor;
//     }
//     return color;
//   };
...
//     // Regular Events Mapping
//     ...regularEvents.map((event) => ({
//       id: event._id,
//       title: event.title || 'Unnamed Event',
//       start: event.start && moment(event.start).isValid()
//         ? moment(event.start).tz('UTC').startOf('day').toDate()
//         : new Date(),
//       end: event.end && moment(event.end).isValid()
//         ? moment(event.end).tz('UTC').endOf('day').toDate()
//         : new Date(),
//       color: validateColor(event.color, '#059669'), // Validate color
//       bgColor: validateColor(event.bgColor, '#ecfdf5'), // Validate bgColor
//       textColor: validateColor(event.textColor, '#FFFFFF'), // Validate textColor
...
//     // Holiday Events Mapping
//     ...holidayEvents.map((holiday) => ({
//       id: holiday._id,
//       title: holiday.title || 'Holiday',
//       start: holiday.start && moment(holiday.start).isValid()
//         ? moment(holiday.start).tz('UTC').startOf('day').toDate()
//         : new Date(),
//       end: holiday.end && moment(holiday.end).isValid()
//         ? moment(holiday.end).tz('UTC').endOf('day').toDate()
//         : new Date(),
//       color: validateColor(holiday.color, '#16a34a'), // Validate color
//       bgColor: validateColor(holiday.bgColor, '#dcfce7'), // Validate bgColor
//       textColor: validateColor(holiday.textColor, '#FFFFFF'), // Validate textColor
...
//     ...meetings
//       .map((meeting) => {
//         const durationStr = meeting.meetingDuration?.toString() || '60 minutes';
//         const durationInMinutes = parseInt(durationStr.split(' ')[0]) || 60;
//         const startMoment = moment(meeting.meetingDate).tz('UTC');

//         if (!startMoment.isValid()) {
//           console.warn("⚠️ Invalid meeting date found:", meeting);
//           return null;
//         }
...
//           color: validateColor(meeting.color, '#7c3aed'), // Validate color
//           bgColor: validateColor(meeting.bgColor, '#f3e8ff'), // Validate bgColor
//           textColor: validateColor(meeting.textColor, '#FFFFFF'), // Validate textColor
...
//         console.log("🎥 Meeting Event Created:", meetingEvent);
//         return meetingEvent;
//       })
//       .filter((event) => event !== null),

//     // Daily Work Mapping
//     ...currentDailyWork.map((work) => ({
//       id: work._id,
//       title: work.title || work.description || 'Unnamed Task',
...
//           : new Date(),
//       color: validateColor(work.color, '#dc2626'), // Validate color
//       bgColor: validateColor(work.bgColor, '#fef2f2'), // Validate bgColor
//       textColor: validateColor(work.textColor, '#FFFFFF'), // Validate textColor
...
//   console.log("✅ Transformed Events (All Types):", JSON.stringify(transformedEvents, null, 2));
...
// }, [getEventsData, getMeetingsData, dailyWork, filterType, searchQuery]);
...
//     // Fix react-big-calendar behavior where end date is exclusive (midnight of the next day)
//     if (slotInfo.action === 'select' && moment(start).isSame(end, 'day') && moment(end).isAfter(moment(start))) {
//       // If it's a click selection on a single day, start and end are the same
//       setSelectedDate(start);
//       setSelectedDateRange(null);
//     } else if (slotInfo.action === 'select' && moment(end).isAfter(moment(start))) {
//       // If it's a drag selection over multiple days
//       const adjustedEnd = moment(end).tz('UTC').subtract(1, 'millisecond').toDate();
//       setSelectedDate(null);
//       setSelectedDateRange({ start, end: adjustedEnd });
//       console.log('Selecting date range (adjusted):', { start, end: adjustedEnd });
//     } else {
//       // Single click on a date cell (day view) or single day selection
//       setSelectedDate(start);
//       setSelectedDateRange(null);
//     }
...
//   console.log("Event Prop Getter:", { id: event.id, title: event.title, color: event.color, bgColor: event.bgColor, textColor: event.textColor });
...
//   // Use the event's custom color for dailyWork, event, or meeting
//   if (event.type === 'dailyWork' || event.type === 'event' || event.type === 'meeting') {
//     return {
//       style: {
//         backgroundColor: event.bgColor, // Use dynamic background
//         border: `2px solid ${event.color}`, // Use dynamic border color
//         borderLeft: `4px solid ${event.color}`,
//         borderRadius: '6px',
//         color: event.textColor, // Dynamic text color
//         fontSize: '12px',
//         fontWeight: '500',
//         padding: '4px 8px',
...
//             <BeachAccessIcon style={{ fontSize: '14px', marginRight: '4px', color: event.color }} />
//             <span style={{
//               color: event.color,
...
//     // Use event.color for the pill background, and event.textColor for the content
//     const iconColor = event.color; 
//     const dynamicTextColor = event.textColor; // ✅ FETCHED: Dynamic text color

//     return (
//       // The overall event block background is set by eventPropGetter (event.bgColor)
//       // This inner div creates the coloured 'pill' effect using event.color as background
//       <div className="flex items-center gap-2 text-xs h-full w-full px-1" 
//            style={{ 
//              backgroundColor: iconColor, // Use accent color as background for the pill
//              borderRadius: '4px', 
//              overflow: 'hidden',
//            }}
//       >
//         {/* Use the dynamic text color */}
//         <IconComponent style={{ fontSize: '14px', color: dynamicTextColor, flexShrink: 0 }} /> 
//         {/* Use the dynamic text color and remove hardcoded text-white class */}
//         <span className="font-semibold truncate" style={{ color: dynamicTextColor }}>
//             {event.title}
//         </span>
//       </div>
//     );
...
//                         <Chip
//                           label={event.type === 'dailyWork' ? 'Task' : event.type}
//                           size="small"
//                           style={{
//                             backgroundColor: event.bgColor,
//                             color: event.color,
...
//                       {event.type === 'dailyWork' && event.guests && event.guests.length > 0 && (
//                         <div className="flex flex-wrap gap-2 text-xs">
//                           <div className="flex items-center gap-1 text-gray-600">
//                             <PersonIcon style={{ fontSize: '12px' }} />
//                             <Tooltip title={event.guests.join(', ')} arrow>
//                               <span className="truncate max-w-xs">Guests: {event.guests.join(', ')}</span>
//                             </Tooltip>
//                           </div>
//                         </div>
//                       )}
...
function CalendarActions({ size, getTimes, getEmployeeName }) {
  ...
  useEffect(() => {
    // Robust check for daily work data
    const rawDailyWork = getDailyWorkData?.data?.data || [];
  ...
  useEffect(() => {
    ...
    console.log("Raw Events Data:", getEventsData?.data?.data);
    console.log("Raw Meetings Data:", getMeetingsData?.data?.data?.data);
    console.log("Raw Daily Work Data:", dailyWork);

    // Safe access with empty array fallback
    const eventsData = Array.isArray(getEventsData?.data?.data) ? getEventsData.data.data : [];
    const meetings = Array.isArray(getMeetingsData?.data?.data?.data) ? getMeetingsData.data.data.data : [];
    const currentDailyWork = Array.isArray(dailyWork) ? dailyWork : [];
    ...
    const transformedEvents = [
      ...
      ...regularEvents.map((event) => ({
        ...
        start: event.start && moment(event.start).isValid()
          ...
          ? moment(event.end).tz('UTC').endOf('day').toDate()
          : new Date(),
        color: validateColor(event.color, '#059669'),
        bgColor: validateColor(event.bgColor, '#ecfdf5'),
        textColor: validateColor(event.textColor, '#FFFFFF'),
        details: `${event.description || 'No description'}`,
        type: 'event',
        createdAt: new Date(event.createdAt || Date.now()),
        icon: EventIcon,
      ...
      ...holidayEvents.map((holiday) => ({
        ...
        start: holiday.start && moment(holiday.start).isValid()
          ...
          ? moment(holiday.end).tz('UTC').endOf('day').toDate()
          : new Date(),
        color: validateColor(holiday.color, '#16a34a'),
        bgColor: validateColor(holiday.bgColor, '#dcfce7'),
        textColor: validateColor(holiday.textColor, '#FFFFFF'),
        details: `${holiday.description || 'Holiday'}`,
        type: 'Holiday',
        createdAt: new Date(holiday.createdAt || Date.now()),
        icon: BeachAccessIcon,
      ...
      ...meetings
        .map((meeting) => {
          ...
          const meetingEvent = {
            ...
            color: validateColor(meeting.color, '#7c3aed'),
            bgColor: validateColor(meeting.bgColor, '#f3e8ff'),
            textColor: validateColor(meeting.textColor, '#FFFFFF'),
            details: `${meeting.meetingAgenda || meeting.meetingDescription || 'No description'}`,
            type: 'meeting',
            host: meeting.meetingHost,
            link: meeting.meetingLink,
      ...
      ...currentDailyWork.map((work) => ({
        ...
        title: work.title || work.description || 'Unnamed Task',
        start: work.startDate && moment(work.startDate).isValid()
          ? moment(work.startDate).tz('UTC').startOf('day').toDate()
          : new Date(),
        end: work.endDate && moment(work.endDate).isValid()
          ? moment(work.endDate).tz('UTC').endOf('day').toDate()
          : work.startDate && moment(work.startDate).isValid()
            ? moment(work.startDate).tz('UTC').endOf('day').toDate()
            : new Date(),
        color: validateColor(work.color, '#dc2626'),
        bgColor: validateColor(work.bgColor, '#fef2f2'),
        textColor: validateColor(work.textColor, '#FFFFFF'),
        details: `${work.description || 'No description'}`,
        type: 'dailyWork',
        createdAt: new Date(work.createdAt || Date.now()),
        icon: AssignmentIcon,
  ...
  const onSelectSlot = (slotInfo) => {
    ...
    if (slotInfo.action === 'select' && moment(start).isSame(end, 'day') && moment(end).isAfter(moment(start))) {
      ...
      // Multi-day selection: Adjust end date to be inclusive
      const adjustedEnd = moment(end).tz('UTC').subtract(1, 'millisecond').toDate();
      setSelectedDate(null);
      setSelectedDateRange({ start, end: adjustedEnd });
      console.log('Selecting date range (adjusted):', { start, end: adjustedEnd });
  ...
  const EventComponent = ({ event }) => {
    ...
    if (event.isHoliday) {
      return (
        <Tooltip title={event.details} arrow>
          <div className="flex items-center justify-center text-green font-bold text-xs h-full">
            <BeachAccessIcon style={{ fontSize: '14px', marginRight: '4px', color: event.color }} />
            <span style={{ color: event.color }}>{event.title.toUpperCase()}</span>
          </div>
        </Tooltip>
      );
    }
    ...
    const iconColor = event.color;
    const dynamicTextColor = event.textColor;

    return (
      <div
        className="flex items-center gap-2 text-xs h-full w-full px-1"
        style={{
